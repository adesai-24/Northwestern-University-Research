# 2/22/23 2D Mega Constellation Simulation by Shaan Doshi and Aadi Desai with guidance from Prof Randall Berry
...
# Define the range of values for each simulation parameter
numSats_range = [50, 75, 100, 125, 150]
numClients_range = [50, 75, 100, 125, 150]
length_range = [250, 375, 500, 625, 750]
width_range = [250, 375, 500, 625, 750]
satAngle_range = [5, 10, 15, 20, 25]
time = 100
...
# Initialize variables to keep track of the best parameter values and their outcomes
best_params = {"numSats": 0, "numClients": 0, "length": 0, "width": 0, "satAngle": 0, "clientAngle": 0, "time": 0}
best_results = {"success": 0, "interference": float("inf"), "noCoverage": float("inf")}

# Iterate through all combinations of parameter values
for numSats in numSats_range:
    for numClients in numClients_range:
        for length in length_range:
            for width in width_range:
                for satAngle in satAngle_range:
                    clientAngle = satAngle
                    ...
                    # Note: replace the default parameter values with the current values in the simulation code
                    # Compute the number of successful connections, interference, and no coverage
                    # Note: the code already stores this information in the 'results' variable
                    # Update the best parameter values and results if the current results are better than the previous best
                    if (results[0]["success"] > best_results["success"] or 
                        (results[0]["success"] == best_results["success"] and results[0]["interference"] < best_results["interference"]) or 
                        (results[0]["success"] == best_results["success"] and results[0]["interference"] == best_results["interference"] and results[0]["noCoverage"] < best_results["noCoverage"])):
                        best_params = {"numSats": numSats, "numClients": numClients, "length": length, "width": width, "satAngle": satAngle, "clientAngle": clientAngle, "time": time}
                        best_results = results[0]

# Output the best parameter values and results
