# 2/1/23 2D Mega Constellation Simulation by Shaan Doshi and Aadi Desai with guidance from Prof Randall Berry
# This code runs the 2D Simulation using two companies and increase probability of companies each iteration. This uses the x axis as probability of company B being chosen
...
numSats = 100
numClients = 100
length = 1000 # how far across the simulation is (X axis)
width = 1000 # how far deep the simulation is (Y axis)
...
probability = [] # an array to hold all the sats
companyAS = []
companyAI = []
companyAN = []
companyBS = []
...
companies = [[.75, 400], [.25, 400]] # company probabilities and heights
...
for a in range(50):
    probability.append(companies[1][0])
    results = [{"success": 0, "interference": 0, "noCoverage": 0} for _ in range(numCompanies)]
    companyClientSizes = [0 for _ in range(numCompanies)]
...
axs.set_xlabel("Probability of company B being chosen")
axs.set_ylabel("Probability")
axs.set_title("Probability of company B being chosen vs Probability of Successes, interference, and No Coverage (sats=100, l=500, w=500)")

plt.plot(probability, companyAI, label="Interference for Company A")
plt.plot(probability, companyAN, label="noCoverage for Company A")
plt.plot(probability, companyAS, label="success for Company A")
plt.plot(probability, companyBI, label="Interference for Company B")
plt.plot(probability, companyBN, label="noCoverage for Company B")
plt.plot(probability, companyBS, label="success for Company B")
